...
const getInstallationToken = cache(async (owner: string, repo: string) => {
  ...
  const repoInstallation = await app.octokit.rest.apps.getRepoInstallation({
    owner,
    repo,
  });
  if (!repoInstallation)
  ...
  if (tokenData && Date.now() < tokenData.expiresAt.getTime() - 60_000) {
    ...
    if (!token)
      throw new Error("Token could not be retrieved and/or decrypted.");
