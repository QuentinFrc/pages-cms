...
import { and, count, eq, gt, inArray, min } from "drizzle-orm";
...
const updateParentFolderCachesBatch = async (
  ...
  const existingDirMap = new Map(
    existingDirEntries.map((dir) => [
      dir.path,
      { parentPath: dir.parentPath, context: dir.context },
    ])
  );

  // 3. Determine additions
  const dirsToInsertData = new Map<
    string,
    typeof cacheFileTable.$inferInsert
  >();
  ...
  if (addedPaths.length > 0) {
    for (const filePath of addedPaths) {
      ...
      const pathsToCheckForAdd =
        directParent === "" ? ancestors : [directParent, ...ancestors];
  ...
  if (deletedPaths.length > 0) {
    ...
    const nonEmptyDirsResult = await db
  ...
  if (operation === "add") {
    ...
    if (dirsToInsert.length > 0) {
      try {
        ...
        console.error(
          "Error inserting single parent dir cache entries:",
          error
        );
      }
    }
...
const updateMultipleFilesCache = async (
  ...
  const pathContextMap = new Map<string, string>();
  if (parentPaths.length > 0) {
    ...
    for (const entry of existingParentEntries) {
      if (
        !pathContextMap.has(entry.parentPath) ||
    ...
    for (const chunk of graphqlChunks) {
      ...
      for (let i = 0; i < chunk.length; i++) {
        ...
        const context =
          parentPath === ""
            ? "collection"
            : pathContextMap.get(parentPath) || "collection";
        const now = new Date();
  ...
  await updateParentFolderCachesBatch(
    owner,
    repo,
    branch,
    addedPaths,
...
const getCollectionCache = async (
  ...
  if (entries.length > 0 && process.env.FILE_CACHE_TTL !== "-1") {
    const now = new Date();
    const ttl =
      Number.parseInt(process.env.FILE_CACHE_TTL || "1440") * 60 * 1000; // Defaults to 7 days cache
  ...
  if (
    ...
    const githubEntries = responseEntries.repository?.object?.entries || [];
  ...
  if (nodeEntryFilename) {
    ...
    if (subdirs.length > 0) {
      ...
      const nodeEntries = await db.query.cacheFileTable.findMany({
        where: and(
          eq(cacheFileTable.owner, owner.toLowerCase()),
          eq(cacheFileTable.repo, repo.toLowerCase()),
          eq(cacheFileTable.branch, branch),
...
const checkRepoAccess = async (
  ...
  const now = new Date();
  const ttl =
    Number.parseInt(process.env.PERMISSION_CACHE_TTL || "60") * 60 * 1000;
