...
const createLoginToken = async (email: string): Promise<string> => {
  await db
    .delete(emailLoginTokenTable)
    .where(eq(sql`lower(${emailLoginTokenTable.email})`, email.toLowerCase()));
...
const getTokenData = async (token: string) => {
  ...
  if (!emailLoginToken)
    throw new Error("Your sign in link is invalid (token is invalid).");

  if (!isWithinExpirationDate(emailLoginToken.expiresAt as Date))
    throw new Error("Your sign in link has expired.");
