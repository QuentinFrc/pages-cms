...
const handleCopyTemplate = async (prevState: any, formData: FormData) => {
  try {
    ...
    const templateRepos = templates.map(
    ...
    if (!templateRepoValidation.success)
      throw new Error("Invalid template repository");

    const ownerAndNameValidation = z
      .object({
    ...
    if (!ownerAndNameValidation.success)
    ...
    const [template_owner, template_repo] =
      templateRepoValidation.data.split("/");
