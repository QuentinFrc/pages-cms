...
export async function GET(request: Request) {
  ...
  try {
    ...
    const fileCacheTTL =
      Number.parseInt(process.env.FILE_CACHE_TTL || "10080") * 60 * 1000;
    ...
    const deletedPermissions = await db
      ...
      .returning();
    console.log(
      `Deleted ${deletedPermissions.length} expired permission cache entries.`
    );
    ...
    return NextResponse.json({
      ...
      deletedPermissions: deletedPermissions.length,
    });
  } catch (error) {
