      ...
      switch (event) {
        case "installation":
          if (data.action === "deleted") {
            ...
            if (!accountLogin) {
              console.error(
                "Missing account login in installation deleted event",
                data.installation
              );
              break;
            }
            ...
            await Promise.all([
              ...
              db
                ...
                .where(
                  eq(
                    githubInstallationTokenTable.installationId,
        ...
        case "installation_repositories":
          if (data.action === "removed") {
            ...
            const reposIdRemoved =
              data.repositories_removed?.map((repo: any) => repo.id) || [];
            if (reposIdRemoved.length === 0) break;

            await db
              .delete(collaboratorTable)
              .where(inArray(collaboratorTable.repoId, reposIdRemoved));
